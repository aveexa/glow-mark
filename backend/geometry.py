...
# Locked ordered feature names — order is part of the contract.
...
class GeometryError(Exception):
    """Domain error for degenerate landmarks or pose outside the frontal band."""
...
def _angle_deg(a: np.ndarray, b: np.ndarray) -> float:
    """Helper: signed 2D angle (degrees) of the segment a→b via atan2."""
    dx = float(b[0] - a[0])
    dy = float(b[1] - a[1])
...
def _triangle_angle_deg(p: np.ndarray, a: np.ndarray, b: np.ndarray) -> float:
    """Helper: interior angle at vertex p of triangle (p, a, b) in degrees."""
    v1 = a - p
    v2 = b - p
...
def assert_frontal(
    ...
    """Pose gate: raise GeometryError if yaw/pitch are outside the frontal band (±25° default)."""
...
def features_to_vector(feats: MutableMapping[str, float]) -> np.ndarray:
    """Ordered float32 vector matching FEATURE_COLS (contract order)."""
