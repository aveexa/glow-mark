...
Called from inference.analyze_image_bytes. Class one-hots default to percentile
p20/p80 thresholds — not Feature MLP argmax. Returns [] if no checkpoint is present.
"""
...
def classes_from_thresholds(
    ...
    """Map continuous feats → low/ok/high via percentile thresholds (not Feature MLP)."""
    out: Dict[str, str] = {}
...
def predict_suggestions(
    ...
    *,
    ...
    not Feature MLP argmax — callers must pass ``y_classes`` explicitly to override.
    """
