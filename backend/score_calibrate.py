...
Env (optional):
  BEAUTY_SCORE_SCALE  default 1.0
  BEAUTY_SCORE_BIAS   default 10.0
...
def _read_calibration() -> Tuple[float, float]:
    ...
    try:
        scale = float(os.environ.get("BEAUTY_SCORE_SCALE", "1.0"))
    except ValueError:
        scale = 1.0
    try:
        bias = float(os.environ.get("BEAUTY_SCORE_BIAS", "10"))
    except ValueError:
        bias = 10.0
    return scale, bias
