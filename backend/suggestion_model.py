"""Suggestion ranker MLP (Phase 10) — train + serve helpers."""
...
def encode_dataset_row(
    row: Mapping[str, Any],
    feat_mu: np.ndarray,
    feat_sd: np.ndarray,
) -> np.ndarray:
    feats = {c: float(row[c]) for c in FEATURE_COLS}
    y_classes = {c: str(row[f"y_{c}"]) for c in FEATURE_COLS}
    return encode_features(feats, y_classes, feat_mu, feat_sd)


def multilabel_target(ids: Sequence[str], vocab: Sequence[str]) -> np.ndarray:
