#!/usr/bin/env python3
"""Bakeoff: BPR / pairwise ranking loss on suggestion scores.

Category: pairwise_ltr

Usage (from repo root):

  python backend/scripts/bakeoff/train_bpr_pairwise.py
"""
...
def bpr_loss(logits: torch.Tensor, y: torch.Tensor, w: torch.Tensor) -> torch.Tensor:
    """Sample one pos + one neg per row; softplus(-(s_pos - s_neg)) × trust."""
    batch, k = logits.shape
    losses = []
    weights = []
    for i in range(batch):
        pos = (y[i] > 0.5).nonzero(as_tuple=False).view(-1)
        neg = (y[i] <= 0.5).nonzero(as_tuple=False).view(-1)
        if pos.numel() == 0 or neg.numel() == 0:
            continue
        pi = pos[torch.randint(0, pos.numel(), (1,), device=logits.device)]
        ni = neg[torch.randint(0, neg.numel(), (1,), device=logits.device)]
        s_pos = logits[i, pi]
        s_neg = logits[i, ni]
        losses.append(F.softplus(-(s_pos - s_neg)))
        weights.append(w[i])
    if not losses:
        return logits.sum() * 0.0
    loss_t = torch.stack(losses).view(-1)
    w_t = torch.stack(weights).view(-1)
    return (loss_t * w_t).sum() / w_t.sum().clamp_min(1e-6)


def main() -> int:
    parser = argparse.ArgumentParser(description="Bakeoff train: BPR pairwise LTR.")
    ...
    def train_fn() -> None:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            print(
                f"epoch {epoch:03d}  train_bpr={train_loss:.4f}  "
