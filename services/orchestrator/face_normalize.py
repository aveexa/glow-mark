...
class FaceCropTransform:
    """Maps crop-normalized XY → original-image-normalized XY.
    ...
    def crop_norm_to_orig_norm(self, nx: float, ny: float) -> Tuple[float, float]:
        """Convert landmark (x,y) in [0,1] of square crop → [0,1] of original image."""
    ...
    def remap_landmarks(self, norm468: np.ndarray) -> np.ndarray:
        """Remap (N,2+) crop-normalized landmarks to original-normalized coords."""
...
def _bbox_from_landmarks(norm468: np.ndarray, w: int, h: int) -> Optional[Tuple[float, float, float, float]]:
    """Return (min_x, min_y, max_x, max_y) in original pixels, or None if degenerate."""
...
def square_face_crop(
    ...
    *,
    ...
    Returns ``(square_bgr, transform)`` or ``None`` on failure (caller fail-softs).
    """
